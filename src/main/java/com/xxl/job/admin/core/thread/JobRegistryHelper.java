...
public class JobRegistryHelper {
            ...
            while (!toStop) {
                try {
                    ...
                    if (groupList != null && !groupList.isEmpty()) {
                        ...
                        // t.update_time <![CDATA[ < ]]> DATE_ADD(#{nowTime},INTERVAL -#{timeout} SECOND)
                        List<Long> ids = XxlJobAdminConfig.getAdminConfig().getXxlJobRegistryDao()
                                .findDead(DateUtil.offsetSecond(new Date(), RegistryConfig.DEAD_TIMEOUT * -1));
                        ...
                        List<XxlJobRegistry> list = XxlJobAdminConfig.getAdminConfig().getXxlJobRegistryDao()
                                .findAll(DateUtil.offsetSecond(new Date(), RegistryConfig.DEAD_TIMEOUT * -1));
                        if (list != null) {
    ...
    public ReturnT<String> registryRemove(RegistryParam registryParam) {
        ...
        // async execute
        registryOrRemoveThreadPool.execute(() -> {
            XxlJobAdminConfig.getAdminConfig().getXxlJobRegistryDao().registryDelete(
                    registryParam.getRegistryGroup(), registryParam.getRegistryKey(), registryParam.getRegistryValue());
            //if (ret > 0) {
            // fresh
            //freshGroupRegistryInfo(registryParam);
            //}
